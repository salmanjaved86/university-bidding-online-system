...
public class AjaxEdit extends ActionSupport implements SessionAware {
    ...
    public boolean validater() {
        ...
        if (Round.getRoundStart()) {
            if (amount.isEmpty()) {
                addActionMessage("Error: Please enter a valid amount");
                return false;
            } else {
                try {
                    ...
                    if (amt <= 0) {
                        ...
                        addActionMessage("Error: Only 2 decimal places allow.");
                        return false;
                    ...
                    if (Round.getRound() == 1) {
                        if (amt < 10.0) {
                            addActionMessage("Error: Your bid is too low. Minimum bid is $10.00");
                            return false;
                        }
                        ...
                        if (minBid == 10.0 && amt < minBid) {
                            addActionMessage("Error: Your bid is too low. Bid placed must be $1 more than the min bid.");
                            return false;
                        } else if (minBid != 10.0 && amt <= minBid) {
                            addActionMessage("Error: Bid placed must be $1 more than the min bid. Current min bid is: $" + minBid);
                            return false;
                        }
                    }
            ...
            if (!checkEditValid()) {
                addActionMessage("Error: Insufficient eDollar.");
                return false;
            }
        } else {
            addActionMessage("Error: Round has already been stopped.");
            return false;
        }
        return true;
    }
