...
public class ViewBids extends ActionSupport implements SessionAware {
    ...
    public String execute() {
        ...
        List<Bid> studentPendingBidList = BidDataManager.getStudentBids(student.getUserId());
        List<SuccessfulBid> studentSuccessfulBidList = SuccessfulBidDataManager.getStudentSuccessfulBids(student.getUserId());
        for (Bid bid : studentPendingBidList) {
            studentBids.add(bid);
        }
        for (SuccessfulBid bid : studentSuccessfulBidList) {
            studentBids.add(new Bid(bid.getUserId(), bid.getCourseCode(), bid.getSectionCode(), bid.getAmount(), false));
        }
        sortBidsForDisplay();
